...
import ve.gob.mppe.redba.pickers.DatePickerDialogFragment;
...
//        dateResultTextView = (TextView) findViewById(R.id.selected_date_text_view);
    ...
    private void processIntent(){
        if(getIntent() != null && getIntent().hasExtra(RedBa.EXTRA_BUNDLE)){
            String result = getIntent().getBundleExtra(RedBa.EXTRA_BUNDLE).getString(RedBa.KEY_RESULT);
            try{
                JSONObject jsonObject = new JSONObject(result);
                subtitle.setText(jsonObject.getString(RedBa.KEY_MESSAGE_TITLE));
                author.setText(jsonObject.getString(RedBa.KEY_MESSAGE_AUTHOR));
